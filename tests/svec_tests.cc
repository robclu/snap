...
BOOST_AUTO_TEST_CASE(can_construct_from_array) {
  svec16x8u vec_from_aligned(uint16x8a);
  svec16x8s vec_from_unaligned(sint16x8u);

  for (auto i = 0; i < 16; ++i) {
    BOOST_CHECK(vec_from_aligned[i]   == uint16x8a[i]);
    BOOST_CHECK(vec_from_unaligned[i] == sint16x8a[i]);
  }
}

BOOST_AUTO_TEST_CASE(can_load_from_aligned_memory) {
  svec16x8u vecu;
  svec16x8s vecs;
...
BOOST_AUTO_TEST_CASE(can_load_from_unaligned_memory) {
  svec16x8u vecu;
  svec16x8s vecs;
...
BOOST_AUTO_TEST_CASE(can_set_single_element) {
  svec16x8u vec(4);   // Set all elements to the value 4.
